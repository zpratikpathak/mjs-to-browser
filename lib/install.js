...
function displaySpec(rawSpec) {
  return rawSpec.endsWith('@latest') ? rawSpec.slice(0, -'@latest'.length) : rawSpec;
}
...
function runNpm(rawSpec, tempDir, { signal, spawnImpl = spawn } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code, childSignal) => {
      ...
      const fullOutput = Buffer.concat(output).toString('utf8');
      if (/\bE404\b|npm error 404\b/i.test(fullOutput)) {
        reject(new CliError(
          `Package "${displaySpec(rawSpec)}" was not found on the npm registry. Check the package name and try again.`,
        ));
        return;
      }
...
async function installPackage(rawSpec, options = {}) {
  ...
  try {
    tempDir = await fsImpl.mkdtemp(path.join(options.tempRoot || os.tmpdir(), 'mjs-to-browser-'));
  } catch (error) {
  ...
  try {
    await fsImpl.writeFile(
      ...
      `${JSON.stringify({ name: 'mjs-to-browser-temporary-project', private: true })}\n`,
      'utf8',
    );
