...
function parsePackageSpec(rawSpec) {
  ...
  if (parsed.type === 'range' && parsed.rawSpec === '*' && parsed.raw === parsed.name) {
    return {
      name: parsed.name,
      raw: `${parsed.name}@latest`,
      version: 'latest',
    };
  }
  ...
  if (!parsed.name || parsed.type !== 'version') {
    throw new UsageError(
      `Package spec must be a registry package name or exact version, for example "fuse.js" or "${parsed.name || 'fuse.js'}@7.5.0".`,
    );
  }
